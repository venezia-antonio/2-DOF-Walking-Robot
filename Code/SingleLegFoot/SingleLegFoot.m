...
TFF_13 = DH(frontFootLength,0,0,pi/2);
% Front Foot
TFF0 = T13_0*TFF_13;
% Transformation matrix BackFoot to Joint 13
TBF_13 = DH(backFootLength,0,0,3*(pi/2));
% Back Foot
TBF0 = T13_0*TBF_13;
...
for i = 1:length(Theta1)
    ...
    FF(:,i) = double(subs(PFF,{theta1 theta2(1) theta3(1) theta4(1) theta5(1) theta6 theta7},{Theta1(i) angle2(i) angle3(i) angle4(i) angle5(i) angle6(i) angle7(i)}));
    BF(:,i) = double(subs(PBF,{theta1 theta2(1) theta3(1) theta4(1) theta5(1) theta6 theta7},{Theta1(i) angle2(i) angle3(i) angle4(i) angle5(i) angle6(i) angle7(i)}));
end
    

%% Load single leg with foot data
%This section is useful in order to not run previous code
load('SingleLegFootData.mat');
...
for i = 1:length(Theta1)
    ...
    h22 = plot([J13(1,i),FF(1,i)],[J13(2,i),FF(2,i)],'b','LineWidth',2);
    h23 = plot([J13(1,i),BF(1,i)],[J13(2,i),BF(2,i)],'b','LineWidth',2);
    hold on
    %Only one leg
    ...
    h26 = plot(FF(1,i),FF(2,i));
    text(FF(1,i),FF(2,i),'FF');
    text(BF(1,i)-0.03,BF(2,i)+0.02,'BF');
    h24 = plot(J13(1,:),J13(2,:),'black:','LineWidth',1.5);
    h25 = plot(FF(1,:),FF(2,:),'black','LineWidth',1.5);
    legend([h1 h9 h15 h16 h5 h24 h25],{'Hinged Joint','Revolute Joint','Hoeken mechanism','Pantograph mechanism','Hoeken''s figure','Gait pattern of ankle','Gait pattern of toe'})
    hold off
    frame = getframe(gcf);
    writeVideo(v,frame);
end
