...
Theta1 = [0:h:2*pi];
Theta1_ = Theta1 + pi;
%% Denavit Hartenberg - Forward Kinematics First Leg
...
%% Denavit Hartenberg - Forward Kinematics Second Leg
...
%% Define COM of each link respect to world frame for the standing leg
...
%% Define COM of each link respect to world frame for the swing leg
...
% Standing Leg
syms Fb1x Fb1y F21x F21y F32x F32y Fb3x Fb3y F42x F42y F64x F64y F54x F54y real
syms Cez Caz real
syms F75x F75y F76x F76y Ff6x Ff6y Fb6x Fb6y Fb7x Fb7y real
syms Ff5x Ff5y real
syms R10x R10y R11x R11y R24x R24y R21x R21y R33x R33y R32x R32y R46x R46y R45x R45y real
syms R44x R44y R5_13x R5_13y R57x R57y R56x R56y R65x R65y R68x R68y R77x R77y R78x R78y real
syms Rb8x Rb8y Rb3x Rb3y Rb0x Rb0y RFoot_13x RFoot_13y real
syms Caz_ Cez_ real

% Swing Leg
syms Fb1x_ Fb1y_ F21x_ F21y_ F32x_ F32y_ Fb3x_ Fb3y_ F42x_ F42y_ F64x_ F64y_ F54x_ F54y_ real
syms F75x_ F75y_ Ff5x_ Ff5y_ Fb6x_ Fb6y_ Fb7x_ Fb7y_ real
syms R10x_ R10y_ R11x_ R11y_ R24x_ R24y_ R21x_ R21y_ R33x_ R33y_ R32x_ R32y_ R46x_ R46y_ R45x_ R45y_ real
syms R44x_ R44y_ R5_13x_ R5_13y_ R57x_ R57y_ R56x_ R56y_ R65x_ R65y_ R68x_ R68y_ R77x_ R77y_ R78x_ R78y_ real
syms Rb8x_ Rb8y_ Rb3x_ Rb3y_ Rb0x_ Rb0y_ RFoot_13x_ RFoot_13y_ real
syms Ff5x_ Ff5y_ real
...
%% Numeric data
% Step of time (change this step to change the angular velocity of motor crank)
% In particular, there are two different angular velocities considered
% w = 0.1 rad/s in which t = 1;
% w = 5 rad/s in which t = 0.02;
t = 1;

% Linear acceleration of standing leg links
a1 = [diff(com1(1,:),2);diff(com1(2,:),2);zeros(1,length(com1)-2)]./t^2;
a2 = [diff(com2(1,:),2);diff(com2(2,:),2);zeros(1,length(com1)-2)]./t^2;
a3 = [diff(com3(1,:),2);diff(com3(2,:),2);zeros(1,length(com1)-2)]./t^2;
a4 = [diff(com4(1,:),2);diff(com4(2,:),2);zeros(1,length(com1)-2)]./t^2;
a5 = [diff(com5(1,:),2);diff(com5(2,:),2);zeros(1,length(com1)-2)]./t^2;
a6 = [diff(com6(1,:),2);diff(com6(2,:),2);zeros(1,length(com1)-2)]./t^2;
a7 = [diff(com7(1,:),2);diff(com7(2,:),2);zeros(1,length(com1)-2)]./t^2;
aBody = [diff(comBody(1,:),2);diff(comBody(2,:),2);zeros(1,length(com1)-2)]./t^2;

% Linear acceleration of swing leg links
a1_ = [diff(com1_(1,:),2);diff(com1_(2,:),2);zeros(1,length(com1)-2)]./t^2;
a2_ = [diff(com2_(1,:),2);diff(com2_(2,:),2);zeros(1,length(com1)-2)]./t^2;
a3_ = [diff(com3_(1,:),2);diff(com3_(2,:),2);zeros(1,length(com1)-2)]./t^2;
a4_ = [diff(com4_(1,:),2);diff(com4_(2,:),2);zeros(1,length(com1)-2)]./t^2;
a5_ = [diff(com5_(1,:),2);diff(com5_(2,:),2);zeros(1,length(com1)-2)]./t^2;
a6_ = [diff(com6_(1,:),2);diff(com6_(2,:),2);zeros(1,length(com1)-2)]./t^2;
a7_ = [diff(com7_(1,:),2);diff(com7_(2,:),2);zeros(1,length(com1)-2)]./t^2;
aFoot_ = [diff(comFoot_(1,:),2);diff(comFoot_(2,:),2);zeros(1,length(com1)-2)]./t^2;

%% Newton-Euler formulation
% Simbolyc variables that defines linear acceleration of all links of the
% walking robot
syms A1x A1y A2x A2y A3x A3y A4x A4y A5x A5y A6x A6y A7x A7y real
syms A1x_ A1y_ A2x_ A2y_ A3x_ A3y_ A4x_ A4y_ A5x_ A5y_ A6x_ A6y_ A7x_ A7y_ AFOOTx_ AFOOTy_ ABODYx ABODYy real


A1 = [A1x A1y 0].';A1_ = [A1x_ A1y_ 0].';
A2 = [A2x A2y 0].';A2_ = [A2x_ A2y_ 0].';
A3 = [A3x A3y 0].';A3_ = [A3x_ A3y_ 0].';
A4 = [A4x A4y 0].';A4_ = [A4x_ A4y_ 0].';
A5 = [A5x A5y 0].';A5_ = [A5x_ A5y_ 0].';
A6 = [A6x A6y 0].';A6_ = [A6x_ A6y_ 0].';
A7 = [A7x A7y 0].';A7_ = [A7x_ A7y_ 0].';
AFOOT_ = [AFOOTx_ AFOOTy_ 0].'; ABODY = [ABODYx ABODYy 0].'; 

% Equations for standing leg

% Equations for L1
eq1 =  Fb1 + F21 + Mass(1).FirstLeg*g == Mass(1).FirstLeg*A1; 
eq11 = cross(R10,Fb1) + cross(R11,F21) + Ca == 0;


% Equations for L2
eq2 = -F21 + F42 + F32 + Mass(2).FirstLeg*g == Mass(2).FirstLeg*A2; 
eq22 = cross(R21,-F21) + cross(R24,F42) == 0;


% Equations for L3
eq3 =  Fb3 - F32 + Mass(3).FirstLeg*g == Mass(3).FirstLeg*A3; 
eq33 = cross(R33,Fb3) + cross(R32,-F32) == 0;

% Equations for L4
eq4 = -F42 + F64 + F54 + Mass(4).FirstLeg*g == Mass(4).FirstLeg*A4; 
eq44 = cross(R44,-F42) + cross(R45,F64) + cross(R46,F54) == 0;

% Equations for L5
eq5 = -F54 + F75 + Ff5 + Mass(6).FirstLeg*g == Mass(6).FirstLeg*A5; 
eq55 = cross(R56,-F54) + cross(R57,F75) + cross(R5_13,Ff5) + Ce == 0;

% Equations for L6
eq6 = -F64 + Fb6 + Mass(5).FirstLeg*g == Mass(5).FirstLeg*A6; 
eq66 = cross(R65,-F64) + cross(R68,Fb6) == 0;

% Equations for L7 
eq7 = Fb7 -F75 + Mass(7).FirstLeg*g == Mass(7).FirstLeg*A7; 
eq77 = cross(R78,Fb7) + cross(R77,-F75) == 0;

% Equations for swing leg

% Equations for L1
eq1_ =  Fb1_ + F21_ + Mass(1).SecondLeg*g == Mass(1).SecondLeg*A1_; 
eq11_ = cross(R10_,Fb1_) + cross(R11_,F21_) + Ca_ == 0;


% Equations for L2
eq2_ = -F21_ + F42_ + F32_ + Mass(2).SecondLeg*g == Mass(2).SecondLeg*A2_; 
eq22_ = cross(R21_,-F21_) + cross(R24_,F42_) == 0;

% Equations for L3
eq3_ =  Fb3_ - F32_ + Mass(3).SecondLeg*g == Mass(3).SecondLeg*A3_; 
eq33_ = cross(R33_,Fb3_) + cross(R32_,-F32_) == 0;

% Equations for L4
eq4_ = -F42_ + F64_ + F54_ + Mass(4).SecondLeg*g == Mass(4).SecondLeg*A4_; 
eq44_ = cross(R44_,-F42_) + cross(R45_,F64_) + cross(R46_,F54_) == 0;

% Equations for L5
eq5_ = -F54_ + F75_ + Ff5_ + Mass(6).SecondLeg*g == Mass(6).SecondLeg*A5_; 
eq55_ = cross(R56_,-F54_) + cross(R57_,F75_) + cross(R5_13_,Ff5_) + Ce_ == 0;

% Equations for L6
eq6_ = -F64_ + Fb6_ + Mass(5).SecondLeg*g == Mass(5).SecondLeg*A6_; 
eq66_ = cross(R65_,-F64_) + cross(R68_,Fb6_) == 0;

% Equations for L7
eq7_ = Fb7_ -F75_ + Mass(7).SecondLeg*g == Mass(7).SecondLeg*A7_; 
eq77_ = cross(R78_,Fb7_) + cross(R77_,-F75_) == 0;

% Equations for swing foot
eq8_ = -Ff5_ + (Mass(8).SecondLeg + Mass(9).SecondLeg)*g == (Mass(8).SecondLeg + Mass(9).SecondLeg)*AFOOT_;
eq88_ = cross(RFoot_13_,-Ff5_) - Ce_ == 0;


% Equations for the hip
eq9 =-Fb1 - Fb3 - Fb6 - Fb7 -Fb1_ - Fb3_ - Fb6_ - Fb7_ + Mass(1).Body*g == Mass(1).Body*ABODY ;
...
A_subs = double(subs(A,{R10x,R10y,R11x,R11y,R24x,R24y,R21x,R21y,R33x,R33y,R32x,R32y,R46x,R46y,R45x,R45y,R44x,R44y,R5_13x,R5_13y,R57x,R57y,R56x,R56y,R65x,R65y,R68x,R68y,R77x,R77y,R78x,R78y,Rb8x,Rb8y,Rb3x,Rb3y,Rb0x,Rb0y,RFoot_13x,RFoot_13y...
                        ...
                        theta1,theta2,theta3,theta4,theta5,theta6,theta7,theta1_,theta2_,theta3_,theta4_,theta5_,theta6_,theta7_,GAMMA,...
                        A1x,A1y,A2x,A2y,A3x,A3y,A4x,A4y,A5x,A5y,A6x,A6y,A7x,A7y,A1x_,A1y_,A2x_,A2y_,A3x_,A3y_,A4x_,A4y_,A5x_,A5y_,A6x_,A6y_,A7x_,A7y_,AFOOTx_,AFOOTy_,ABODYx,ABODYy},...
                        ...
                        Theta1(i),angle2(i),angle3(i),angle4(i),angle5(i),angle6(i),angle7(i),Theta1_(i),angle2_(i),angle3_(i),angle4_(i),angle5_(i),angle6_(i),angle7_(i),gamma(i),...
                        a1(1,i),a1(2,i),a2(1,i),a2(2,i),a3(1,i),a3(2,i),a4(1,i),a4(2,i),a5(1,i),a5(2,i),a6(1,i),a6(2,i),a7(1,i),a7(2,i),a1_(1,i),a1_(2,i),a2_(1,i),a2_(2,i),a3_(1,i),a3_(2,i),a4_(1,i),a4_(2,i),a5_(1,i),a5_(2,i),a6_(1,i),a6_(2,i),a7_(1,i),a7_(2,i),aFoot_(1,i),aFoot_(2,i),aBody(1,i),aBody(2,i)}));

b_subs = double(subs(b,{R10x,R10y,R11x,R11y,R24x,R24y,R21x,R21y,R33x,R33y,R32x,R32y,R46x,R46y,R45x,R45y,R44x,R44y,R5_13x,R5_13y,R57x,R57y,R56x,R56y,R65x,R65y,R68x,R68y,R77x,R77y,R78x,R78y,Rb8x,Rb8y,Rb3x,Rb3y,Rb0x,Rb0y,RFoot_13x,RFoot_13y...
                        R10x_,R10y_,R11x_,R11y_,R24x_,R24y_,R21x_,R21y_,R33x_,R33y_,R32x_,R32y_,R46x_,R46y_,R45x_,R45y_,R44x_,R44y_,R5_13x_,R5_13y_,R57x_,R57y_,R56x_,R56y_,R65x_,R65y_,R68x_,R68y_,R77x_,R77y_,R78x_,R78y_,Rb8x_,Rb8y_,Rb3x_,Rb3y_,Rb0x_,Rb0y_,RFoot_13x_,RFoot_13y_...
                        theta1,theta2,theta3,theta4,theta5,theta6,theta7,theta1_,theta2_,theta3_,theta4_,theta5_,theta6_,theta7_,GAMMA,...
                        A1x,A1y,A2x,A2y,A3x,A3y,A4x,A4y,A5x,A5y,A6x,A6y,A7x,A7y,A1x_,A1y_,A2x_,A2y_,A3x_,A3y_,A4x_,A4y_,A5x_,A5y_,A6x_,A6y_,A7x_,A7y_,AFOOTx_,AFOOTy_,ABODYx,ABODYy},...
                        {r10(1,i),r10(2,i),r11(1,i),r11(2,i),r24(1,i),r24(2,i),r21(1,i),r21(2,i),r33(1,i),r33(2,i),r32(1,i),r32(2,i),r46(1,i),r46(2,i),r45(1,i),r45(2,i),r44(1,i),r44(2,i),r5_13(1,i),r5_13(2,i),r57(1,i),r57(2,i),r56(1,i),r56(2,i),r65(1,i),r65(2,i),r68(1,i),r68(2,i),r77(1,i),r77(2,i),r78(1,i),r78(2,i),rBody8(1,i),rBody8(2,i),rBody3(1,i),rBody3(2,i),rBody0(1,i),rBody0(2,i),rFoot13(1,i),rFoot13(2,i)...
...
solution(:,j) = inv(A_subs)*b_subs;
j=j+1;
% This disp is useful to know how much time it takes
disp(i)
end

%% Compute actuated torque Cm and elastic torque Ce
...
% ke is obteined considering the slope of regression line
ke = -p(1);
...
%% First simulation
% angular velocity of crank equal to 0.1 rad/s
% Mass of body equal to 16 kg
% COM of body equal to the position of J8 
load('FirstSimulation.mat');

%% Second simulation
% angular velocity of crank equal to 0.1 rad/s
% Mass of body equal to 16 kg
% COM of body equal to the position of J8 + 0.3 m
load('SecondSimulation.mat');

%% Third simulation
% angular velocity of crank equal to 0.1 rad/s
% Mass of body equal to 30 kg
% COM of body equal to the position of J8 + 0.3 m
load('ThirdSimulation.mat');

%% Fourth simulation
% angular velocity of crank equal to 5 rad/s
% Mass of body equal to 20 kg
% COM of body equal to the position of J8 + 0.3 m
load('FourthSimulation.mat');

%% Movie
figure('units','normalized','outerposition',[0 0 1 1])
view(2)
set(gca,'nextplot','replacechildren');
v = VideoWriter('Dynamic-FirstSimulation.avi');
open(v);
hold on
grid on
for i = 27:49
    ...
    grid on
    t = tiledlayout(2,2);
    title(t,'Inverse Dynamics - First Simulation','Color','r')
    ax1 = nexttile([2 1]);
    grid on
    axis equal
