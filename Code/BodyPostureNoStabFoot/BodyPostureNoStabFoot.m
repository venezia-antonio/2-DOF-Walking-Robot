...
for i=1:length(Theta1)
    R = TFF0(1:3,1:3);
    RFF0 = double(subs(R,{theta1,theta2,theta3,theta4,theta5,theta6,theta7},{Theta1(1,i),angle2(1,i),angle3(1,i),angle4(1,i),angle5(1,i),angle6(1,i),angle7(1,i)}));
    alfa(i) = AngoloDaTraccia(RFF0);
end
...
BF = double(subs(PBFW,{theta1,theta2,theta3,theta4,theta5,theta6,theta7,ALFA},{Theta1,angle2,angle3,angle4,angle5,angle6,angle7,alfa}));
JW = [0 0 0]';
...
% Substitution of joint variables
...
FF_ = double(subs(PFFW_,{theta1,theta2,theta3,theta4,theta5,theta6,theta7,theta1_,theta2_,theta3_,theta4_,theta5_,theta6_,theta7_,ALFA},...
        {Theta1,angle2,angle3,angle4,angle5,angle6,angle7,Theta1_,angle2_,angle3_,angle4_,angle5_,angle6_,angle7_,alfa}));
    
BF_ = double(subs(PBFW_,{theta1,theta2,theta3,theta4,theta5,theta6,theta7,theta1_,theta2_,theta3_,theta4_,theta5_,theta6_,theta7_,ALFA},...
        {Theta1,angle2,angle3,angle4,angle5,angle6,angle7,Theta1_,angle2_,angle3_,angle4_,angle5_,angle6_,angle7_,alfa}));
...
% It contains all data of two legs (including feet) referred to base frame 0 
load('BodyPostureNoStabFoot.mat');

%% Movie
figure('units','normalized','outerposition',[0 0 1 1])
view(2)
set(gca,'nextplot','replacechildren');
v = VideoWriter('BodyPostureNoStabFoot.avi');
open(v);
hold on
grid on
for i = 27:57
    ...
    title('Two-Legged Walking Robot with feet');
    %Vertices of hip
    A = [J0(1,i),J0(2,i)];
    B = [A(1)+L0,A(2)];
    C = [A(1)+L0,A(2)-L13];
    ...
    h12 = plot([J13(1,i),JW(1,1)],[J13(2,i),JW(2,1)],'b','LineWidth',2);
    h13 = plot([J13(1,i),BF(1,i)],[J13(2,i),BF(2,i)],'b','LineWidth',2);
    hold on
    ...
    text(J13(1,i)+0.02,J13(2,i)+0.01,'13');
    h25 = plot(J13(1,27:57),J13(2,27:57),'m:','LineWidth',1.5)
    ...
    h36 = plot([J13_(1,i),FF_(1,i)],[J13_(2,i),FF_(2,i)],'r','LineWidth',2);
    h37 = plot([J13_(1,i),BF_(1,i)],[J13_(2,i),BF_(2,i)],'r','LineWidth',2);
    hold on
    ...
    h49 = plot(FF_(1,27:57),FF_(2,27:57),'black','LineWidth',1.5);
    %h50 = plot(J13_(1,27:55),J13_(2,27:55),'black:','LineWidth',1.5);
    text(JW(1,1),JW(2,1)-0.02,'FF');
    text(FF_(1,i),FF_(2,i),'FF''');
    h51 = plot([0 0.1],[0 0],'m','Linewidth',1.5);
    h52 = plot([0 0],[0 0.1],'m','Linewidth',1.5);
    h53 = plot(0.1,0,'m>','Linewidth',1.5);
    h54 = plot(0,0.1,'m^','Linewidth',1.5);
    text(0.1,-0.02,'x','Color','m');
    text(0.02,0.12,'y','Color','m');
    h55 = plot(com(1,i),com(2,i),'o','Color',[0 0.5 0],'Linewidth',1.5);
    h56 = plot(com(1,27:57),com(2,27:57),'Color',[0 0.5 0],'Linewidth',1.5);
    legend([h38 h39 h1 h55 h2 h26 h25 h56 h49 h51],{'Hinged Joint','Revolute Joint','Hip','COM of hip','Standing leg','Swing leg','Path of point 13','Hip trajectory','Gait pattern swing foot','World Frame'});
    hold off  
    frame = getframe(gcf);
    writeVideo(v,frame);
end
